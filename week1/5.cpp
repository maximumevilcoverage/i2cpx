...
public:
    ...
    void (*doit)(vi&,X&);
    X& out;
    Permute(vi mapping, X& out, void (doit)(vi&,X&)):mapping(mapping),doit(doit),out(out){
        v = vul(mapping.size());
    ...
    void permute(){
        if (perm.size() == v.size()){
            vi permutation;
            for (auto e:perm){
                permutation.PB(mapping[e]);
            }
            doit(permutation,out);
        }
        else{
            LR(i,v.size()){
                if (!chosen[i]){
...
/* Permute example usage:
 *
    int max;
    Permute<int> p({0,1,2}, max, [](vi a, int& b){b = 1; prtl(a);});
...
// how to create a vector of vectors: vvi matrix(rows, vi(columns));
/* INPUT TYPES
...
int main(){
    ...
    int max=0;
    Permute<int> p({0,1,2}, max, [](vi& a, int& b)
    {
        b = pmax(b,sqr(m[0][a[0]])+sqr(m[1][a[1]])+sqr(m[2][a[2]]));
    });
    prt(sqrt(max));
}
