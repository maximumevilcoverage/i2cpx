...
template <typename T>
T amin(T x, T y){ if (abs(x) < abs(y)){ return abs(x); } else { return abs(y); } }
template <typename T>
...
template <typename T>
vector<T> zipreduce(const vector<T>& a, const vector<T>& b, T (op)(T,T)){
    return zipreduce(a, b, 0, op);
}
...
int main(){
    ...
    int n; gli(n);
    vi a; vi b; gli(a);gli(b);
    vi diffs = zipreduce(a,b,psub);
    vi maxes = zipreduce(a,b,pmax);
    int s= SUM(maxes);
    int dmin= MIN(diffs);
    int dmax= MAX(diffs);
    if (dmax * dmin > 0) s -= amin(dmax,dmin);
    prt(s);
}
